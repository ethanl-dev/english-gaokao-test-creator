...
4.5 **超纲词筛查与处理**
   ...
   改编语料后，必须对每篇文章进行超纲词筛查：

   **参考标准**：《普通高中英语课程标准》（2017 年版 2025 年修订）规定的词汇表。

   **筛查方法**：逐篇检查文章中可能超出课标词汇表的单词（重点检查实词：名词、动词、形容词、副词），对照课标词汇表判断是否为超纲词。

   **处理规则**（按优先级）：

   | 类别 | 判定条件 | 处理方式 |
   |------|----------|----------|
   | A 类 - 可忽略 | 不影响文章主旨理解，也不影响学生做题 | 不做任何处理，保留原文 |
   | B 类 - 题型豁免 | 词义推测题的考察对象本身 | 不做中文标注 |
   | C 类 - 必须标注 | 影响学生理解题目或选出正确答案，且不是题目考察对象 | 在该词后加括号注中文翻译，如 `subsidy (补贴)` |

   **硬性限制**：每篇文章 C 类中文标注不得超过 3 个。若超过 3 个，说明文章词汇难度超标，必须对部分超纲词进行同义词替换，将 C 类标注数控制在 3 个以内。

   **命题阶段注意**：在步骤 5 编写词义推测题时，该题考察的超纲词自动归为 B 类，不在文中标注中文。
...
- `heading`：大题标题
- `instructions`：答题说明
- `passage`：正文（可选），按空行分段。超纲词中文标注格式：`word (中文)` 紧跟超纲词后，如 `The government provided a subsidy (补贴) for renewable energy projects.`
...
- 每篇文章中文标注超纲词（C 类）不超过 3 个；超过则必须替换后重新检查。
- 词义推测题考察的超纲词未在原文中标注中文翻译。
